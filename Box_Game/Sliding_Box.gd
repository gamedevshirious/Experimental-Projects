...
remote func move(l_speed) :	
	# linear_velocity += delta*GRAVITY_VECTOR
	linear_velocity = move_and_slide(l_speed, FLOOR_NORMAL, SLOPE_SLIDE_STOP)

func _physics_process(delta):
	# $TrailPart.emitting = false
	if is_network_master() :
		linear_velocity += GRAVITY_VECTOR
		# linear_velocity = move_and_slide(linear_velocity, FLOOR_NORMAL, SLOPE_SLIDE_STOP)
		rpc_unreliable("move",linear_velocity)
		var target_speed = 0
		if Input.is_action_pressed("ui_left"):
			target_speed -= 1
			$TrailPart.emitting = true
