...
func load_shape():
	...
	while true:
		...
		if file == "":
			break
		elif not (file.begins_with("_") or file.begins_with("bean") or file.begins_with(".")) :
			heroes.append(file)
	dir.list_dir_end()
	load_curr_hero()
...
func _on_SinglePlayer_pressed():
	...
	print_debug(globals.curr_hero)
	globals.mode = "singleplayer"
	globals.player_name = $TypeSelector/PlayerNameLabel.text
	return get_tree().change_scene("res://game_scenes/levels/Dojo.tscn")
